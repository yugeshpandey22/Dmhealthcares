<section class="testimonials-section py-5 bg-white" id="testimonials">
    <div class="container py-5">
        <div class="text-center mb-5">
            <span class="text-uppercase fw-bold text-secondary mb-2 d-block" style="letter-spacing: 1.5px; font-size: 0.85rem;">Testimonials</span>
            <h2 class="section-title fw-bolder mb-3" style="color: var(--primary-color); font-size: 2.5rem;">What Our Patients Say</h2>
            <div style="width: 60px; height: 4px; background: var(--secondary-color); margin: 0 auto 20px auto; border-radius: 2px;"></div>
            <p class="section-subtitle text-muted fs-6 mx-auto" style="max-width: 600px;">Real stories of care, compassion, and recovery from verified Google reviews.</p>
        </div>

        <div class="row g-4 justify-content-center">
            <!-- Testimonial 1 -->
            <div class="col-md-6 col-lg-4">
                <div class="card h-100 border-0 shadow-sm rounded-4 p-4 review-card">
                    <div class="d-flex align-items-center mb-3">
                        <div class="rounded-circle d-flex align-items-center justify-content-center fw-bold fs-5 me-3 text-white" style="width: 50px; height: 50px; background-color: #EA4335;">R</div>
                        <div>
                            <h6 class="fw-bold mb-0 text-dark">Ramesh Kumar</h6>
                            <div class="d-flex align-items-center gap-2 mt-1">
                                <div class="text-warning small" style="letter-spacing: 2px;">
                                    <i class="fa-solid fa-star"></i><i class="fa-solid fa-star"></i><i class="fa-solid fa-star"></i><i class="fa-solid fa-star"></i><i class="fa-solid fa-star"></i>
                                </div>
                                <span class="text-muted small" style="font-size: 0.75rem;">3 weeks ago</span>
                            </div>
                        </div>
                        <i class="fa-brands fa-google ms-auto fs-4" style="color: #4285F4;"></i>
                    </div>
                    <p class="text-dark fs-6" style="line-height: 1.6;">"We hired a 24/7 nurse for my mother after her surgery. The staff from DM Healthcare was incredibly professional, kind, and always on time. Highly recommended for home care."</p>
                </div>
            </div>

            <!-- Testimonial 2 -->
            <div class="col-md-6 col-lg-4">
                <div class="card h-100 border-0 shadow-sm rounded-4 p-4 review-card">
                    <div class="d-flex align-items-center mb-3">
                        <div class="rounded-circle d-flex align-items-center justify-content-center fw-bold fs-5 me-3 text-white" style="width: 50px; height: 50px; background-color: #34A853;">A</div>
                        <div>
                            <h6 class="fw-bold mb-0 text-dark">Anjali Singh</h6>
                            <div class="d-flex align-items-center gap-2 mt-1">
                                <div class="text-warning small" style="letter-spacing: 2px;">
                                    <i class="fa-solid fa-star"></i><i class="fa-solid fa-star"></i><i class="fa-solid fa-star"></i><i class="fa-solid fa-star"></i><i class="fa-solid fa-star"></i>
                                </div>
                                <span class="text-muted small" style="font-size: 0.75rem;">1 month ago</span>
                            </div>
                        </div>
                        <i class="fa-brands fa-google ms-auto fs-4" style="color: #4285F4;"></i>
                    </div>
                    <p class="text-dark fs-6" style="line-height: 1.6;">"The medical equipment rental service was a lifesaver. We got a motorized hospital bed and an oxygen concentrator delivered and installed within hours of our request."</p>
                </div>
            </div>

            <!-- Testimonial 3 -->
            <div class="col-md-6 col-lg-4">
                <div class="card h-100 border-0 shadow-sm rounded-4 p-4 review-card">
                    <div class="d-flex align-items-center mb-3">
                        <div class="rounded-circle d-flex align-items-center justify-content-center fw-bold fs-5 me-3 text-white" style="width: 50px; height: 50px; background-color: #FBBC05;">S</div>
                        <div>
                            <h6 class="fw-bold mb-0 text-dark">Sanjay Mishra</h6>
                            <div class="d-flex align-items-center gap-2 mt-1">
                                <div class="text-warning small" style="letter-spacing: 2px;">
                                    <i class="fa-solid fa-star"></i><i class="fa-solid fa-star"></i><i class="fa-solid fa-star"></i><i class="fa-solid fa-star"></i><i class="fa-solid fa-star"></i>
                                </div>
                                <span class="text-muted small" style="font-size: 0.75rem;">2 months ago</span>
                            </div>
                        </div>
                        <i class="fa-brands fa-google ms-auto fs-4" style="color: #4285F4;"></i>
                    </div>
                    <p class="text-dark fs-6" style="line-height: 1.6;">"Thanks to the expert physiotherapy team at DM Healthcare, my father regained his mobility completely after his stroke. The dedication of the staff is unmatched."</p>
                </div>
            </div>
        </div>

        <div class="text-center mt-5">
            <a href="#" class="btn btn-outline-dark rounded-pill px-4 fw-semibold d-inline-flex align-items-center gap-2">
                <i class="fa-brands fa-google" style="color: #4285F4;"></i> Read more on Google
            </a>
        </div>
    </div>
</section>

<style>
.review-card {
    transition: all 0.3s ease;
    border: 1px solid rgba(0,0,0,0.05) !important;
}
.review-card:hover {
    transform: translateY(-5px);
    box-shadow: 0 15px 30px rgba(0,0,0,0.08) !important;
}
</style>
