<section class="specialized-section py-5" style="background-color: #f8fafd;" id="treatments">
    <div class="container py-5">
        <div class="text-center mb-5">
            <h2 class="fw-bolder mb-3" style="color: #1a2b4c; font-size: 2.8rem;">Specialized <span style="color: var(--primary-color);">Treatments</span></h2>
            <div style="width: 80px; height: 4px; background: var(--secondary-color); margin: 0 auto 20px auto; border-radius: 2px;"></div>
            <p class="text-muted fs-5">Expert care plans designed for specific medical conditions.</p>
        </div>

        <div class="row g-4 mt-2">
            <!-- Treatment 1 -->
            <div class="col-md-6 col-lg-4">
                <div class="card h-100 border-0 rounded-4 shadow-sm overflow-hidden treatment-card" style="width: 18rem; margin: 0 auto;">
                    <div class="position-relative overflow-hidden" style="height: 200px;">
                        <img src="assets/images/downloaded_img_1.jpg" class="card-img-top treatment-img" alt="Orthopedic Care">
                        <div class="position-absolute bottom-0 start-0 w-100 p-3" style="background: linear-gradient(to top, rgba(0,0,0,0.8), transparent);">
                            <div class="d-flex align-items-center gap-2 text-white">
                                <i class="fa-solid fa-bone fs-4 text-warning"></i>
                                <h5 class="mb-0 fw-bold">Orthopedic Rehab</h5>
                            </div>
                        </div>
                    </div>
                    <div class="card-body p-4 bg-white">
                        <p class="text-muted mb-4">Comprehensive post-surgery care and physiotherapy to help you regain mobility and strength after joint replacements or fractures.</p>
                        <a href="#appointment" class="btn btn-outline-primary rounded-pill px-4 fw-bold w-100">Book Care Plan</a>
                    </div>
                </div>
            </div>

            <!-- Treatment 2 -->
            <div class="col-md-6 col-lg-4">
                <div class="card h-100 border-0 rounded-4 shadow-sm overflow-hidden treatment-card" style="width: 18rem; margin: 0 auto;">
                    <div class="position-relative overflow-hidden" style="height: 200px;">
                        <img src="assets/images/downloaded_img_2.jpg" class="card-img-top treatment-img" alt="Neurological Care">
                        <div class="position-absolute bottom-0 start-0 w-100 p-3" style="background: linear-gradient(to top, rgba(0,0,0,0.8), transparent);">
                            <div class="d-flex align-items-center gap-2 text-white">
                                <i class="fa-solid fa-brain fs-4 text-warning"></i>
                                <h5 class="mb-0 fw-bold">Neurological Care</h5>
                            </div>
                        </div>
                    </div>
                    <div class="card-body p-4 bg-white">
                        <p class="text-muted mb-4">Specialized home care for patients recovering from strokes, managing Parkinson's, or dealing with Alzheimer's and Dementia.</p>
                        <a href="#appointment" class="btn btn-outline-primary rounded-pill px-4 fw-bold w-100">Book Care Plan</a>
                    </div>
                </div>
            </div>

            <!-- Treatment 3 -->
            <div class="col-md-6 col-lg-4">
                <div class="card h-100 border-0 rounded-4 shadow-sm overflow-hidden treatment-card" style="width: 18rem; margin: 0 auto;">
                    <div class="position-relative overflow-hidden" style="height: 200px;">
                        <img src="assets/images/downloaded_img_3.jpg" class="card-img-top treatment-img" alt="Cardiac Recovery">
                        <div class="position-absolute bottom-0 start-0 w-100 p-3" style="background: linear-gradient(to top, rgba(0,0,0,0.8), transparent);">
                            <div class="d-flex align-items-center gap-2 text-white">
                                <i class="fa-solid fa-heart-pulse fs-4 text-warning"></i>
                                <h5 class="mb-0 fw-bold">Cardiac Recovery</h5>
                            </div>
                        </div>
                    </div>
                    <div class="card-body p-4 bg-white">
                        <p class="text-muted mb-4">Close monitoring of heart patients post-surgery with ECG setups, oxygen support, and specialized lifestyle management.</p>
                        <a href="#appointment" class="btn btn-outline-primary rounded-pill px-4 fw-bold w-100">Book Care Plan</a>
                    </div>
                </div>
            </div>

            <!-- Treatment 4 -->
            <div class="col-md-6 col-lg-4">
                <div class="card h-100 border-0 rounded-4 shadow-sm overflow-hidden treatment-card" style="width: 18rem; margin: 0 auto;">
                    <div class="position-relative overflow-hidden" style="height: 200px;">
                        <img src="assets/images/downloaded_img_4.jpg" class="card-img-top treatment-img" alt="Pulmonary Care">
                        <div class="position-absolute bottom-0 start-0 w-100 p-3" style="background: linear-gradient(to top, rgba(0,0,0,0.8), transparent);">
                            <div class="d-flex align-items-center gap-2 text-white">
                                <i class="fa-solid fa-lungs fs-4 text-warning"></i>
                                <h5 class="mb-0 fw-bold">Pulmonary Care</h5>
                            </div>
                        </div>
                    </div>
                    <div class="card-body p-4 bg-white">
                        <p class="text-muted mb-4">Expert respiratory therapy and care for COPD, Asthma, and post-COVID recovery, including oxygen administration.</p>
                        <a href="#appointment" class="btn btn-outline-primary rounded-pill px-4 fw-bold w-100">Book Care Plan</a>
                    </div>
                </div>
            </div>

            <!-- Treatment 5 -->
            <div class="col-md-6 col-lg-4">
                <div class="card h-100 border-0 rounded-4 shadow-sm overflow-hidden treatment-card" style="width: 18rem; margin: 0 auto;">
                    <div class="position-relative overflow-hidden" style="height: 200px;">
                        <img src="assets/images/downloaded_img_5.jpg" class="card-img-top treatment-img" alt="Post-Surgical Care">
                        <div class="position-absolute bottom-0 start-0 w-100 p-3" style="background: linear-gradient(to top, rgba(0,0,0,0.8), transparent);">
                            <div class="d-flex align-items-center gap-2 text-white">
                                <i class="fa-solid fa-bed-pulse fs-4 text-warning"></i>
                                <h5 class="mb-0 fw-bold">Post-Surgical Care</h5>
                            </div>
                        </div>
                    </div>
                    <div class="card-body p-4 bg-white">
                        <p class="text-muted mb-4">Dedicated wound dressing, infection prevention, and daily monitoring to ensure a safe and fast recovery at home.</p>
                        <a href="#appointment" class="btn btn-outline-primary rounded-pill px-4 fw-bold w-100">Book Care Plan</a>
                    </div>
                </div>
            </div>

            <!-- Treatment 6 -->
            <div class="col-md-6 col-lg-4">
                <div class="card h-100 border-0 rounded-4 shadow-sm overflow-hidden treatment-card" style="width: 18rem; margin: 0 auto;">
                    <div class="position-relative overflow-hidden" style="height: 200px;">
                        <img src="assets/images/downloaded_img_6.jpg" class="card-img-top treatment-img" alt="Palliative Care">
                        <div class="position-absolute bottom-0 start-0 w-100 p-3" style="background: linear-gradient(to top, rgba(0,0,0,0.8), transparent);">
                            <div class="d-flex align-items-center gap-2 text-white">
                                <i class="fa-solid fa-hand-holding-heart fs-4 text-warning"></i>
                                <h5 class="mb-0 fw-bold">Palliative Care</h5>
                            </div>
                        </div>
                    </div>
                    <div class="card-body p-4 bg-white">
                        <p class="text-muted mb-4">Compassionate end-of-life care focusing on pain relief, comfort, and emotional support for both patients and families.</p>
                        <a href="#appointment" class="btn btn-outline-primary rounded-pill px-4 fw-bold w-100">Book Care Plan</a>
                    </div>
                </div>
            </div>

        </div>
    </div>
</section>

<style>
    .treatment-card {
        transition: all 0.4s cubic-bezier(0.175, 0.885, 0.32, 1.275);
    }
    .treatment-card:hover {
        transform: translateY(-10px);
        box-shadow: 0 15px 30px rgba(0,0,0,0.1) !important;
    }
    .treatment-img {
        width: 100%;
        height: 100%;
        object-fit: cover;
        transition: transform 0.6s ease;
    }
    .treatment-card:hover .treatment-img {
        transform: scale(1.1);
    }
</style>

